...
class CheckFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    monitor_id = None
    check_time = factory.Sequence(lambda n: datetime.datetime.now() - datetime.timedelta(hours=1*n))
    status_code = factory.LazyAttribute(lambda o: STATUS_CODE_LIST[o._status_int])
